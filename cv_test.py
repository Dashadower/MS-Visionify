...
def _find_minimap_area(original_image, best_rect_area, debug = False):
    """Input: BGR image. Output: estimed bounding box(x,y,w,h) of minimap area"""
    processed_img = cv2.cvtColor(original_image, cv2.COLOR_BGR2GRAY)
    processed_img = cv2.GaussianBlur(processed_img, (3, 3), 3)
    processed_img = cv2.erode(processed_img, (7,7))
...
while True:
    ...
    _processed_minimap_img = _find_minimap_area(minimap_roi_img, best_area, True)
    ...
    cv2.imshow("dilated", imutils.resize(processed_minimap_img[2], width=300))
    cv2.imshow("eroded&dilated", imutils.resize(_processed_minimap_img[2], width=300))
